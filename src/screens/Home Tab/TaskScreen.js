...
const TaskScreen = ({ navigation }) => {
  ...
  const getData = () => {
    ...
    frappe.get_list('Task',filters=[["Task","_assign","like","%kamesh@erevive.in%"]], fields=["*"],start=start_limit).then((resp)=>{
      ...
      resp.data.forEach(a => {
        // console.log(a)
        mapped_array.push({data:a, doctype:'Task', title: a.name, subtitle: `${a.first_name} ${a?.last_name ? a?.last_name : ''}`, 
  ...
  const searchFilterFunction = (text) => {
    setloading(true)
    frappe.get_list('Task',filters=[["Task","name","like",`%${text}%`]], fields=["*"]).then((resp)=>{
      console.log(resp)
      setloading(false)
      if(resp.data){
        mapped_array = []
